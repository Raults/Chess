...
public class Moves {
    ...
    //Never actually get around to using this code, only testing it.
    //The undo is an extra feature and not necessary though.
    ...
    public static String posibleMoves() {
        String list="";//Method that reads all possible moves methods
        for (int i=0; i<64; i++) {
    ...
    public static String posibleA(int i) {
        String list="", cyst="", oldPiece, oldCastle;
        int r=i/8, c=i%8;
        for (int j=0;j<9;j++) {
            if (j!=4) {
                try {
                ...
                /*if (ABChess.humanAsWhite==1){}
                try {//Castling
                    if (" ".equals(chessBoard[r][c-1]) &&
                            " ".equals(chessBoard[r][c-2]) &&
                            " ".equals(chessBoard[r][c-3]) &&
                            "R".equals(chessBoard[r][c-4])){
                        oldPiece=chessBoard[r][c-2];
                        chessBoard[r][c]=" ";
                        chessBoard[r][c-2]="A";
                        int kingTemp=kingPositionC;
                        kingPositionC=[r][c-2]
                        if (kingSafe()) {
                            list=list+r+c+r+(c-2)+oldPiece;
                        }
                        chessBoard[r][c]="A";
                        chessBoard[r][c-2]=oldPiece;
                        kingPositionC=kingTemp;
                    }
                } catch (Exception e) {}*/
            }
    ...
    /*
    public static boolean castleCheck(int v) {
        if (v==0){
            int temp=1;
            for (int i=-1; i<=1; i+=2) {
                try {
